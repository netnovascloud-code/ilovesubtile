    ...
    <div className="space-y-5">
      {!file ? (
        ...
        <div className="flex items-center justify-between rounded-lg border border-ink-100 bg-white px-4 py-2.5 text-sm">
          ...
          <button onClick={() => { if (bmpRef.current) { bmpRef.current.close(); bmpRef.current = null; } setFile(null); setBmp(null); setOutUrl(null); }} aria-label="Remove" className="rounded p-1 text-ink-400 hover:bg-ink-50 hover:text-ink-700"><X className="h-3.5 w-3.5" /></button>
        </div>
      )}
