...
// dev) can be added by setting WYRLO_EXTRA_ORIGINS as a comma-separated env.
...
function allowedOrigin(req: Request): string {
  ...
  if (/^https:\/\/[a-z0-9-]+(?:--wyrlo)?\.vercel\.app$/.test(o)) return o;
  const extra = (Deno.env.get("WYRLO_EXTRA_ORIGINS") ?? "").split(",").map((s) => s.trim()).filter(Boolean);
