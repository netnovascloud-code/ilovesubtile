// Konvertools smoke test (v28) — runs in GitHub Actions against a freshly-built
...
// ── Pass 2.6 — favicon, apple-icon and web manifest ───────────────────────
// Generated by app/icon.tsx, app/apple-icon.tsx and app/manifest.ts. Verify
// the browser tab icon and PWA install metadata actually serve.
for (const [label, path, want] of [
  ...
  try {
    ...
    const ct = r.headers.get("content-type") || "";
    if (!r.ok) throw new Error(`HTTP ${r.status}`);
    if (!want.test(ct)) throw new Error(`content-type=${ct}`);
    record(label, true);
  } catch (e) {
    record(label, false, (e instanceof Error ? e.message : String(e)).slice(0, 160));
  }
}
