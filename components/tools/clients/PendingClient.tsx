    ...
    <div className="space-y-4">
      {!file ? (
        <label className={cn("flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed px-6 py-12 text-center transition-colors hover:brightness-95", th.dropBorder, th.dropBg)}>
          ...
          <span className="mt-3 font-semibold text-ink-900">Click to upload</span>
          <span className="mt-0.5 text-xs text-ink-400">Accepted: {accept || "any file"}</span>
      ...
      <div className="rounded-xl border border-amber-200 bg-amber-50/60 p-4">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm">
            <p className="font-semibold text-amber-900">Bientôt disponible</p>
            <p className="mt-0.5 text-amber-800">This tool runs server-side. We're finalising the conversion engine — drop in any time and we'll let you know when it's live.</p>
          </div>
        </div>
      </div>
      ...
      <p className="text-xs text-ink-400">When live: files are deleted immediately after your download. Never stored, never shared.</p>
    </div>
  );
}
