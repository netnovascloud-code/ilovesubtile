...
const en: FaqFactory[] = [
  (name) => ({ q: `Is ${name} free to use?`, a: `Yes. Wyrlo is free for everyone. Free users can run ${name} a few times per day with a file size limit. Pro users (€12/month) get unlimited runs, larger files, and no ads.` }),
...
const fr: FaqFactory[] = [
  (name) => ({ q: `${name} est-il gratuit ?`, a: `Oui. Wyrlo est gratuit pour tous. Les utilisateurs gratuits peuvent utiliser ${name} plusieurs fois par jour avec une limite de taille. Les Pro (12 €/mois) ont un usage illimité, des fichiers plus gros et aucune pub.` }),
...
const es: FaqFactory[] = [
  (name) => ({ q: `¿${name} es gratis?`, a: `Sí. Wyrlo es gratis para todos. Los usuarios gratuitos pueden usar ${name} varias veces al día con un límite de tamaño. Pro (12 €/mes) ofrece usos ilimitados, archivos grandes y sin anuncios.` }),
...
const pt: FaqFactory[] = [
  (name) => ({ q: `${name} é grátis?`, a: `Sim. Wyrlo é grátis para todos. Usuários gratuitos podem usar ${name} algumas vezes ao dia com um limite de tamanho. Pro (€12/mês) tem usos ilimitados, arquivos maiores e sem anúncios.` }),
...
const de: FaqFactory[] = [
  (name) => ({ q: `Ist ${name} kostenlos?`, a: `Ja. Wyrlo ist für alle kostenlos. Gratis-Nutzer können ${name} mehrmals täglich mit Dateigrößenlimit verwenden. Pro (12 €/Monat) hebt das auf, erlaubt größere Dateien und entfernt Werbung.` }),
...
const it: FaqFactory[] = [
  (name) => ({ q: `${name} è gratis?`, a: `Sì. Wyrlo è gratis per tutti. Gli utenti gratis possono usare ${name} alcune volte al giorno con un limite di dimensione. Pro (12 €/mese) offre utilizzi illimitati, file più grandi e niente pubblicità.` }),
...
const nl: FaqFactory[] = [
  (name) => ({ q: `Is ${name} gratis?`, a: `Ja. Wyrlo is gratis voor iedereen. Gratis gebruikers kunnen ${name} enkele keren per dag draaien met een grootte-limiet. Pro (€12/maand) geeft onbeperkt gebruik, grotere bestanden en geen advertenties.` }),
...
const ja: FaqFactory[] = [
  (name) => ({ q: `${name} は無料ですか？`, a: `はい。Wyrlo は誰でも無料です。無料ユーザーは ${name} を 1 日数回まで、ファイルサイズ制限ありで利用可。Pro（月 12 ユーロ）は無制限、より大きなファイル、広告なしです。` }),
...
const zh: FaqFactory[] = [
  (name) => ({ q: `${name} 是免费的吗?`, a: `是的,Wyrlo 对所有人免费。免费用户每天可使用 ${name} 数次,有文件大小限制。Pro 用户(€12/月)享有无限使用、更大文件、无广告。` }),
...
const ko: FaqFactory[] = [
  (name) => ({ q: `${name} 는 무료인가요?`, a: `네. Wyrlo 는 모두에게 무료입니다. 무료 사용자는 ${name} 를 하루 몇 번, 파일 크기 제한 하에 사용할 수 있습니다. Pro(월 12유로)는 무제한 사용, 더 큰 파일, 광고 없음.` }),
...
const ar: FaqFactory[] = [
  (name) => ({ q: `هل ${name} مجاني؟`, a: `نعم. Wyrlo مجاني للجميع. يمكن للمستخدم المجاني تشغيل ${name} عدة مرات يوميًا مع حد لحجم الملف. Pro (12 يورو/شهر) يمنح استخدامًا غير محدود، ملفات أكبر، وبلا إعلانات.` }),
...
const ru: FaqFactory[] = [
  (name) => ({ q: `${name} бесплатно?`, a: `Да. Wyrlo бесплатен для всех. Бесплатные пользователи могут запускать ${name} несколько раз в день с лимитом на размер файла. Pro (12 €/мес) даёт безлимит, большие файлы и без рекламы.` }),
...
const hi: FaqFactory[] = [
  (name) => ({ q: `क्या ${name} मुफ़्त है?`, a: `हाँ। Wyrlo सभी के लिए मुफ़्त है। मुफ़्त उपयोगकर्ता ${name} को रोज़ कुछ बार उपयोग कर सकते हैं, फ़ाइल आकार सीमा के साथ। Pro (€12/माह) असीमित उपयोग, बड़ी फ़ाइलें, और कोई विज्ञापन नहीं देता है।` }),
