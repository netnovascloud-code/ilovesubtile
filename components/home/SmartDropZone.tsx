  ...
  return (
    <div className="mx-auto max-w-2xl rounded-xl border border-ink-200 bg-white p-4 shadow-card">
      <div className="flex items-center justify-between gap-2">
        ...
        <button onClick={() => setFile(null)} className="rounded p-1 text-ink-400 hover:bg-ink-50 hover:text-ink-700"><X className="h-3.5 w-3.5" /></button>
      </div>
      {suggestions.length === 0 ? (
        <p className="mt-3 text-sm text-ink-500">No tool matches a <code>.{ext}</code> file yet — try the search below.</p>
      ) : (
